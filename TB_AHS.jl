...
const household = Array{Household}(undef,20000)
...
function reduce_density()
    new_density()
    dist_new_housing()
end
function get_new_size(lambda,housing)
    new_size = rand(Poisson(lambda),housing)
    No_zero(new_size)
    remain = length(findall(x->x.Hid>7156,human))
    dif = sum(new_size) - remain
    if dif > 0
        z = findall(x->x>2,new_size)
        r = sample(z,dif,replace=false)
        for i in r
            new_size[i]-=1
        end
    elseif dif < 0
...
function new_density()
    lambda = p.lambda
    housing = p.housing
    new_size = get_new_size(lambda,housing)
    remain_people = findall(x->x.Hid>7156,human)
    new_total = 7156+housing
    new_size_min =  findmin(new_size)[1]
    new_size_max =  findmax(new_size)[1]
    a = Array{Int16}(undef,new_size_max)

    for i in 1:new_size_max
        a[i] = length(findall(x->x==i,new_size))
    end

    Smin = Array{Int16}(undef,new_size_max)
    Smax = Array{Int16}(undef,new_size_max)
    Smin[1] = 1+7156
    Smax[1] = a[1]+7156

    for i in 2:new_size_max
        Smin[i] = sum(a[1:(i-1)])+1+7156
        Smax[i] = sum(a[1:i])+7156
    end
    
    for i in 7157:new_total
        household[i] = Household()
        household[i].Hid = i
    end
    
    for H in new_size_min:new_size_max
        for j in Smin[H]:Smax[H]
            household[j].Size = H
            household[j].AvailableSize=H
        end
...
function dist_new_housing()
    hh_members = findall(x->x.Hid>7156,human) #finds individuals who live in crowded households
    
    for i in hh_members  #reset their household id to zero
        human[i].Hid = 0
    end

    #Distributing at least one adult in each housing
    adults = sample(filter(x -> x.AgeGroup >= 4 && x.Hid==0,human),p.housing,replace=false)
    count = 7156
    for i in 1:length(adults)
        count+=1
        adults[i].Hid = count
        adults[i].treated = rand()<p.θ ? true : adults[i].treated
        household[count].AvailableSize -= 1
    end
